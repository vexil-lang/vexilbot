...
**Container registry:** `ghcr.io/vexil-lang/vexilbot`
...
- **vexstore/** — Append-only `.vxb` binary store (magic header + schema hash + length-prefixed records)
- **logstore/** — `slog.Handler` that persists log records to vexstore as typed `LogEntry` messages
...
1. **Server-side** (`config.toml`): listen address, webhook secret, GitHub App ID + private key path, cargo registry token, Anthropic API key, data directory for `.vxb` stores
...
- Binary runs as non-root (uid 65532) — mount a writable volume for `data_dir` in config
- `docker-compose.yml` is the Caddy-based reference for reverse proxy + TLS
- `docker-compose.override.yml` for local dev: `vexilbot:local` image, port 8080 exposed directly, Caddy skipped via profile
- If using Traefik with Cloudflare: DNS record for the webhook subdomain must be **DNS only (grey cloud)**, not proxied, so Traefik can complete ACME challenges
...
- `.vxb` store files are written to `data_dir` (config field, default `"data"`); mount a persistent volume in Docker or the data is lost on container restart
